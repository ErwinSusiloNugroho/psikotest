...
// Ambil data peserta dari database berdasarkan user_id di session
$peserta_data = null;
try {
    ...
    if (!$peserta_data) {
        // Jika data tidak ditemukan, redirect ke form identitas
        header("Location: form-identitas.php");
        exit();
    }
    ...
    if (empty($errors)) {
        try {
            ...
            foreach ($soal_list as $soal) {
                ...
                // Validasi jawaban hanya YA atau TIDAK
                if (!in_array($jawaban, ['YA', 'TIDAK'])) {
                    throw new Exception("Jawaban tidak valid untuk soal " . $soal['nomor_soal']);
                }
            ...
            <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h3 class="font-semibold text-gray-800"><?php echo htmlspecialchars($peserta_data['nama']); ?></h3>
                            <p class="text-sm text-gray-600">
                                <?php echo htmlspecialchars($peserta_data['jenis_kelamin']); ?> • 
                                <?php echo htmlspecialchars($peserta_data['usia']); ?> tahun • 
                                <?php echo htmlspecialchars($peserta_data['pekerjaan']); ?>
                            </p>
                        </div>
                    </div>
